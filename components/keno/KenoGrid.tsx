...
const NumberButton: React.FC<NumberButtonProps> = ({ num, onClick, isSelected, isRevealed, isWinning, isHit, isFinished, isIdle }) => {
    ...
    if (isFinished) {
        if (isHit) {
            ...
            baseClasses += ' bg-[#f44336]/30 border-2 border-red-500'; // Missed selection
        } else if (isWinning) {
            baseClasses += ' bg-gray-600/50'; // Winning number not selected
            content = <span className="text-green-400">{num}</span>;
        } else {
            baseClasses += ' bg-[#2c353d] opacity-50'; // Not involved
        }
    } else if (isRevealed) {
        if (isHit) {
